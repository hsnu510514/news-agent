...
describe("Dashboard Page - Analysis Stats & Live Progress Panel", () => {
  beforeEach(() => {
    mockFetch.mockReset();
    vi.stubEnv("NEXT_PUBLIC_API_URL", "http://localhost:8000");
  });
  ...
  it("renders the Live Progress Panel when an AI Analysis task is active", async () => {
    mockFetch.mockImplementation((url: string) => {
  ...
  it("renders backlog status when no task is running (idle)", async () => {
    mockFetch.mockImplementation((url: string) => {
      if (url.includes("/api/tasks/analysis-stats")) {
        return Promise.resolve({
          ok: true,
    ...
    // Verify backlog is displayed without progress bar
    expect(await screen.findByText(/System Idle/i)).toBeInTheDocument();
    expect(screen.getByText(/Backlog: 25 news articles pending/i)).toBeInTheDocument();
    expect(screen.queryByText(/Processed: 8 \/ 20/i)).not.toBeInTheDocument();
  });
});
